...
public class DoEditNews implements Command {
	...
	private static final String NEWS_EDITED_SUCCES = "local.globalMessage.edited";
	...
	private static final String S_E_MESS = "Sorry, problems with update";
	private static final String SE_MESSAGE = "message";
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!validator.getErrors().isEmpty()) {
			...
			Integer news_id = (int) session.getAttribute(NEWS_ID);
			try {
				newsService.update(new News(news_id, request.getParameter(NEWS_TITLE), request.getParameter(NEWS_BRIEF),
						request.getParameter(NEWS_CONTENT), request.getParameter(NEWS_DATE), userId));
				session.setAttribute(GLOBAL_MESSAGE, NEWS_EDITED_SUCCES);
				response.sendRedirect(COMMAND_GO_TO_NEWS_LIST);
			} catch (ServiceException e) {
				session.setAttribute(SE_MESSAGE, S_E_MESS);
				response.sendRedirect(COMMAND_GO_TO_ERROR_PAGE);
